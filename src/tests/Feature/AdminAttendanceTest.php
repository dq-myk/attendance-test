...
class AdminAttendanceTest extends TestCase
{
    ...
    //管理者当日と前日と翌日の勤怠情報確認
    public function test_admin_previous_and_following_day()
    {
        $staff = User::factory()->create([
            'role' => 'staff',
        ...
        $this->actingAs($admin)
            ->get("/admin/attendance/list?year={$currentDate->year}&month=" . sprintf('%02d', $currentDate->month) . "&day=" . sprintf('%02d', $currentDate->day))
            ->assertStatus(200)
            ->assertSee($currentDate->year)
            ->assertSee(sprintf('%02d', $currentDate->month))
            ->assertSee(sprintf('%02d', $currentDate->day))
            ->assertSee('01:00')
            ->assertSee('08:00');

        $this->actingAs($admin)
            ->get("/admin/attendance/list?year={$previousDate->year}&month=" . sprintf('%02d', $previousDate->month) . "&day=" . sprintf('%02d', $previousDate->day))
            ->assertStatus(200)
            ->assertSee($previousDate->year)
            ->assertSee(sprintf('%02d', $previousDate->month))
            ->assertSee(sprintf('%02d', $previousDate->day))
        ...
        $this->actingAs($admin)
            ->get("/admin/attendance/list?year={$nextDate->year}&month=" . sprintf('%02d', $nextDate->month) . "&day=" . sprintf('%02d', $nextDate->day))
            ->assertStatus(200)
            ->assertSee($nextDate->year)
            ->assertSee(sprintf('%02d', $nextDate->month))
            ->assertSee(sprintf('%02d', $nextDate->day))
    ...
    //管理者出勤時間が退勤時間より後になっている場合
    public function test_admin_attendance_update_work_time_error()
    {
        [$staff, $attendance, $formattedDate] = $this->createStaffAttendanceList();
    ...
    //管理者休憩開始時間が退勤時間より後の場合
    public function test_admin_attendance_update_rest_start_time_error()
    {
        [$staff, $attendance, $formattedDate] = $this->createStaffAttendanceList();
        ...
        $response = $this->put("/admin/attendance/{$attendance->id}", $restData);

        $response->assertSessionHasErrors(['custom_error' => '出勤時間もしくは退勤時間が不適切な値です']);
    }

    //管理者休憩終了間が退勤時間より後の場合
    public function test_admin_attendance_update_rest_end_time_error()
    {
        [$staff, $attendance, $formattedDate] = $this->createStaffAttendanceList();
    ...
    //管理者備考の入力必須
    public function test_admin_remarks_is_required()
    {
        [$staff, $attendance, $formattedDate] = $this->createStaffAttendanceList();
    ...
    public function test_admin_attendance_staff()
    {
        ...
        $this->actingAs($admin)
            ->get("/admin/attendance/staff/{$staff->id}?month={$currentDate->month}&year={$currentDate->year}")
            ->assertStatus(200)
            ->assertSee($staff->name)
            ->assertSee($currentDate->year)
            ->assertSee(sprintf('%02d', $currentDate->month))
        ...
        $this->actingAs($admin)
            ->get("/admin/attendance/staff/{$staff->id}?month={$previousDate->month}&year={$previousDate->year}")
            ->assertStatus(200)
            ->assertSee($staff->name)
            ->assertSee($previousDate->year)
            ->assertSee(sprintf('%02d', $previousDate->month))
        ...
        $this->actingAs($admin)
            ->get("/admin/attendance/staff/{$staff->id}?month={$nextDate->month}&year={$nextDate->year}")
            ->assertStatus(200)
            ->assertSee($staff->name)
            ->assertSee($nextDate->year)
            ->assertSee(sprintf('%02d', $nextDate->month))
    ...
    public function test_attendance_detail_page()
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertStatus(200)
            ->assertSee('詳細')
            ->assertSee('/admin/attendance/' . $attendance->id);
    ...
    //スタッフ修正処理
    public function test_admin_attendance_request()
    {
    ...
    public function test_admin_attendance_approve_screen()
    {
        [$staff, $attendance, $formattedDate] = $this->createStaffAttendanceList();
        $rest = $attendance->rests->first();
        ...
        $attendanceCorrectRequest = AttendanceCorrectRequest::where('attendance_id', $attendance->id)->latest()->first();
        ...
        $approveResponse = $this->actingAs($admin)->post("/stamp_correction_request/approve/{$attendanceCorrectRequest->id}", [
            'status' => '承認済み'
        ]);

        $approveResponse->assertStatus(302); // リダイレクトされるはず
